...
func GetIdentifiers(stream chan (*pb.Identifier)) error {
	...
	for key, doc := range docs {
		...
		stream <- identifier //ok, want to be passing these out on a channel.
	}
	stream <- nil
	return nil
}

func GetTypeInfo(in *pb.STRING, ch chan (*pb.Identifier)) error {
	...
	if t.Kind() != reflect.Interface {
		...
		for i := 0; i < t.NumMethod(); i++ {
			sendMethodIdentifier(t.Method(i), ch)
		}

		tptr := reflect.PointerTo(t)
		for i := 0; i < tptr.NumMethod(); i++ {
			sendMethodIdentifier(tptr.Method(i), ch)
		}
	}
...
func sendMethodIdentifier(method reflect.Method, ch chan (*pb.Identifier)) {
...
func sendFieldIdentifier(field reflect.StructField, ch chan (*pb.Identifier)) {
	identifier := &pb.Identifier{
		Name: field.Name,
